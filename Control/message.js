...
const { imageToWebp, videoToWebp, writeExif } = require('../Control/exif');
const { isUrl, getGroupAdmins, generateMessageTag, getBuffer, getSizeMedia, fetchJson, sleep, getTypeUrlMedia } = require('../Control/Function');
...
async function MessagesUpsert(conn, message, store) {
	try {
		...
		require('./Commands/case.js')(conn, m, message, store);
...
conn.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	...
	conn.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		...
		const trueFileName = attachExtension ? `./Data/sampah/${filename ? filename : Date.now()}.${type.ext}` : filename;
	...
	conn.getFile = async (PATH, save) => {
		...
		filename = path.join(__filename, '../Data/sampah/' + new Date * 1 + '.' + type.ext)
...
async function Serialize(conn, m, store) {
	...
	if (m.key) {
		...
		if (m.isGroup) {
			...
			m.isAdmin = m.admins ? (m.admins.includes(m.sender) || false) : false
